-- ljsocket: a LuaJIT FFI socket library, originally by CapsAdmin, in the form bundled with
-- AutoSubs (github.com/tmoroney/auto-subs, MIT). One change for Resolve's Console: every ffi.cdef
-- is wrapped in pcall, so starting the bridge a second time in the same Lua state does not fail on
-- a type redeclaration. Third-party notice in LICENSE.
...
do
    ...
    if ffi.os == "Windows" then
        ...
        do
            pcall(ffi.cdef, [[int GetLastError();]])
        ...
        do
            pcall(ffi.cdef, [[int WSAStartup(uint16_t version, void *wsa_data);]])
        ...
        if jit.arch ~= "x64" then -- xp or something
            pcall(ffi.cdef, [[int WSAAddressToStringA(struct sockaddr *, unsigned long, void *, char *, unsigned long *);]])
        ...
        do
            pcall(ffi.cdef, [[int ioctlsocket(SOCKET s, long cmd, unsigned long* argp);]])
    ...
    do
        pcall(ffi.cdef, [[const char *inet_ntop(int __af, const void *__cp, char *__buf, unsigned int __len);]])
    ...
    do
        pcall(ffi.cdef, [[SOCKET socket(int af, int type, int protocol);]])
    ...
    if ffi.os == "OSX" then
        ...
        e.SO_NOSIGPIPE = 0x1022 -- macOS has no MSG_NOSIGNAL send flag; the socket option does the job
...
do
    ...
    function meta:send(data, flags, addr)
        ...
        if not len then
            -- same mapping receive() and accept() apply: a full send buffer on a non-blocking
            -- socket is "timeout", not a failure (added for the Resolve bridge)
            if not self.blocking and timeout_messages[num] then
                return nil, "timeout", num
            end
            return len, err, num
